...
public class SampleDAO {
	...
	private static String nameList ="고수,공유,권상우,권율,김범,김수현,김우빈,"
								+ "남주혁,박보검,박서준,박해일,박해진,박형식,서강준,서인국,소지섭"
								+ "송강호, 신구, 안재욱,양택조,옥택연,원기준"
								+ "원빈,유오성,유지태,윤계상,윤상현,이계인"
								+ "이광기,이덕화,이동욱,이병헌,이원준,이준기"
								+ "인교진,임창정,임채무,전무송,전국환,전노민,"
								+ "정우성,정보석,정용인,조인성,조정석,조진웅"
	...
	static {
		...
		for(int i=0; i <names.length; i++) {
			...
			// 위에서 생성한 연락처 정보를 목록에 하나씩 담음
			// contacts.get(i)로 하나씩 꺼낼 수 있고
			// contacts.get(i)의 내용은 {1, 고수, 010-1111-2210, 서울특별시} 이런 구성
			contacts.add(i, c);
		}
	}
	// 전체 연락처 목록 조회
	public static ContactList getContacts() {
		// static 블록에서 생성한 연락처 목록으로 된 ContactList 객체 생성 순서 : 현재페이지,페이지사이즈, 목록사이즈, List<Contact> contacts
	...
	// 연락처 수정
	// 수정 될 연락처 정보 매개변수로 받음
	public static int updateContact(Contact c) {
		long no = c.getNo(); // 일련번호 가지고옴
		// 수정된 연락처의 개수
		int count = 0;
		for(int i=0; i < contacts.size(); i++) {
			// 기존의 연락처 목록에서 연락처 하나씩 꺼내서 비교
			Contact con = contacts.get(i);
			// 매개변수로 받은 연락처의 일련번호와 기존 연락처 목록에서 꺼내온 연락처의 일련번호가 같을 때 
			if(con.getNo() == no) {
	...
	public static int updateBatch(ContactList contactList) {
		// 수정 될 연락처의 개수
		int count = 0;
		// 매개변수에서 pageN0, pageSize, totalCount는 제외한 연락처 목록만 뽑음
		List<Contact> list = contactList.getContacts();
		if(list.size() > 0) {
			// 연락처 목록에서 연락처 정보를 하나씩 꺼냄
			for(Contact c : list) {
				// 수정 메소드 호출
